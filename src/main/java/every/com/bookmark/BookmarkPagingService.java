...
public class BookmarkPagingService {
	...
	// 1. currentPage(현재 페이지)를 받아서 현재 페이지에 띄워줄 navi의 범위를 결정하는일
			...
			public HashMap<String, Object> getPageNavi(int currentPage, String id) throws Exception{
				// 1. 전체 데이터가 몇개인지 뽑아내는 작업
				int recordTotalCnt = dao.countAll(id); // 전체 데이터 수
