...
/* 관리자 페이지 검색에 대한 리스트를 불러오는 페이징 */
	...
	// 1. currentPage(현재 페이지)를 받아서 현재 페이지에 띄워줄 navi의 범위를 결정하는일
		...
		public HashMap<String, Object> getPageNavi(int currentPage, String searchKey, String selected) throws Exception{
			// 1. 전체 데이터가 몇개인지 뽑아내는 작업
			int recordTotalCnt = dao.searchCountAll(searchKey, selected); // 전체 데이터 수
