...
<%-- 			</c:choose> --%>
	...
	<div class="main">
		...
		<div class="row infoDiv" id="tab">
			<div class="col-4 tabmenu current" data-tab="userInfo" id="info">공지사항</div>
			<div class="col-4 tabmenu" data-tab="userFaq" id="faq">자주묻는 질문</div>
			<div class="col-4 tabmenu" data-tab="userQna" id="qna">문의 조회</div>
		</div>

		<!-- 공지사항 영역 -->
		<div class="tabContents infoDiv current" id="userInfo">
		...
		<!-- 자주묻는 질문 영역 -->
		<div class="tabContents infoDiv" id="userFaq">
			<div style="width: 80%; margin: auto;">
				<div class="row mt-4">
		...
		<!-- 문의조회 영역 -->
		<div class="tabContents infoDiv" id="userQna">
			<div style="width: 80%; margin: auto;">
				<div class="row mt-4">
					<div class="col-3 suv_title_div">
						<h3>Q&A</h3>
...
<div class="modal fade" id="bellModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
	...
	function getInfoList() {
		$.ajax({
			...
			if(rs == "") {
				...
				let infoList = rs.list;
				for(list of infoList) {
					let infoList = "<tr><td style='text-align: center;'>"+
								   "<a href='${pageContext.request.contextPath}/info/toDetail.do?seq_info=" + list.seq_info + "'>" + list.info_title + "</a></td>" +
			...
			if(rs == "") {
				...
				for(list of userFaq) {
					let faqList = "<tr><td style='text-align: center;'>"+
								   "<a href='${pageContext.request.contextPath}/faq/toDetail.do?seq_faq=" + list.seq_faq + "'>" + list.faq_title + "</a></td>" +
					  			   "<td style='text-align: center;'>" + list.admin_id + "</td>" +
	...
	function getQnaList() {
		$.ajax({
			...
			if(rs == "") {
				...
				$("#myList").append(qnaList);
			} else {
				let myQna = rs.list;
				for(list of myQna) {
					let qnaList = "<tr><td style='text-align: center;'>"+
								   "<a href='${pageContext.request.contextPath}/qna/toDetail.do?seq_qna=" + list.seq_qna + "'>" + list.seq_qna + "</a></td>" +
					  			   "<td style='text-align: center;'>" + list.qna_title + "</td>" +
   ...
   $('.tabmenu').click(function() {
      ...
      if(tab_id == "userInfo") {
    	  getInfoList();
      } else if(tab_id == "userFaq") {
    	  getFaqList();
      }  else if(tab_id == "userQna") {
    	  getQnaList();
      }
